...
public class UsefulParScoring {
    ...
    */
    ...
    public static long compute(String X, String Y, int xLength, int yLength, int iter) {
        ...
        char[] Stringx = new char[xLength+1], Stringy = new char[yLength+1];


        //init row
        for(int ii = 1; ii < xLength + 1; ii++){
            S[ii][0] = M[1][0]*ii;//M[1][0]*ii;
            Stringx[ii] = X.charAt(ii-1);
        }

        //init column
        for(int jj = 1; jj < yLength+1; jj++){
            S[0][jj] = M[0][1]*jj;;//M[0][1]*jj;
            Stringy[jj] = Y.charAt(jj-1);
        }
        //init diagonal
        S[0][0] = 0;
        ...
        int i = 1, j =1;
        int[] jay =  new int[1], eye = new int[1];
        jay[0] = 1;
        eye[0] = 1;

        while(eye[0] < xLength || jay[0] < yLength){

            char XChar = X.charAt(i - 1);//i-1
            char YChar = Y.charAt(j - 1);//-1


            //fill out the diagonal before going row and column
            int diagScore = S[i - 1][j - 1] + M[charMap(XChar)][charMap(YChar)];//[i - 1][j - 1]
            int topColScore = S[i - 1][j] + M[charMap(XChar)][0];//[i - 1][j]
            int leftRowScore = S[i][j - 1] + M[0][charMap(YChar)];//[i][j - 1]
            ...
            int jj = jay[0];//j;
            //THIS IS CHANGING THE COLUMN

            finish(() ->{
                forasyncChunked(eye[0], xLength, (ii) -> {

                    int XdiagScore = S[ii - 1][jj - 1] + M[charMap(Stringx[ii])][charMap(Stringy[jj])];//[i - 1][j - 1]
                    int XtopColScore = S[ii - 1][jj] + M[charMap(Stringx[ii])][0];//[i - 1][j]
                    int XleftRowScore = S[ii][jj - 1] + M[0][charMap(Stringy[jj])];//[i][j - 1]

                    S[ii][jj] = Math.max(XdiagScore, Math.max(XleftRowScore, XtopColScore));
                ...
                //THIS IS CHANGING THE ROW

                forasyncChunked(jay[0], yLength, (yy) -> {//forasyncChunked

                    int YdiagScore = S[zz - 1][yy - 1] + M[charMap(Stringx[zz])][charMap(Stringy[yy])];//[i - 1][j - 1]
                    int YtopColScore = S[zz - 1][yy] + M[charMap(Stringx[zz])][0];//[i - 1][j]
                    int YleftRowScore = S[zz][yy - 1] + M[0][charMap(Stringy[yy])];//[i][j - 1]

                    S[zz][yy] = Math.max(YdiagScore, Math.max(YleftRowScore, YtopColScore));
